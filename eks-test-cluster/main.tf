terraform {
  ...
  backend "s3" {
    bucket    = "tofu-state"
    key       = "eks-test-cluster/terraform.tfstate"
...
module eksa-test-cluster {
  source = "../../delivery-nutanix-iac/modules/eks-d"

  cluster_name = "test-2"
  control_plane_cert_sans = ["kube-test.mtsi.bigbang.dev", "10.0.200.60"]
  control_plane_host = "10.0.200.60"
  kube_version = "1.29"
